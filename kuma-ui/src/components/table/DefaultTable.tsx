...
export function DefaultTable<T extends DataItem>({
	...
	const confirmDelete = () => {
		...
		dataId.forEach(async (id) => {
			try {
				...
				setData(data.filter((d) => !dataId.includes(d.id)));
			} catch (error) {
				console.error("Error:", error);
			}
			setShowAlert(false);
	...
	const handleDelete = () => {
		...
		setDataId(selectedRows);
		setShowAlert(true);
	};

	return (
		<div className="w-full pr-12">
			<Button onClick={handleCreate} className="capitalize">
				Add {endpoint}
			</Button>
			<Button onClick={handleDelete} className="capitalize ml-6" disabled={!Object.keys(rowSelection).length}>
				Delete {endpoint}
			</Button>
			{isCreate || isEdit ? (
				<FormComponent data={editData} onClose={closeForm} />
			) : (
				<div className="w-full">
					<div className="flex items-center py-4">
						<Input
							placeholder={`Filter ${filterDescription}...`}
							value={filter}
